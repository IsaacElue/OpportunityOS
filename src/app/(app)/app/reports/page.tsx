...
function ReportCard({ report }: { report: ReportRow }) {
  ...
  return <Card className="p-5 sm:p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-medium text-brand">Founder Opportunity Report</p><h2 className="mt-1 text-lg font-semibold">{opportunity?.title ?? "Opportunity analysis"}</h2></div><span className="rounded-xl border border-brand/20 bg-brand/10 px-3 py-2 text-sm font-semibold text-brand">{report.confidence_score ?? "-"}</span></div><p className="mt-4 line-clamp-3 text-sm leading-6 text-muted">{report.problem ?? "Scout is still preparing the problem summary."}</p><div className="mt-5 flex items-center justify-between gap-3 border-t border-white/10 pt-4"><time className="text-xs text-muted" dateTime={report.created_at}>{new Intl.DateTimeFormat("en", { month: "short", day: "numeric", year: "numeric" }).format(new Date(report.created_at))}</time>{scanId ? <Link href={`/app/scans/${scanId}`} className="inline-flex items-center gap-1 text-sm font-medium text-brand hover:text-[#c2ffda]">Open investigation <ArrowUpRight className="size-4" /></Link> : null}</div></Card>;
}
