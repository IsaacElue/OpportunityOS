...
export function ScoutBriefingCard({ briefing, className }: { briefing: ScoutBriefing | null; className?: string }) {
  ...
  return <section className={cn("scout-surface relative overflow-hidden rounded-3xl p-5 sm:p-6", className)} aria-labelledby={headingId}><div className="pointer-events-none absolute -right-16 -top-20 size-48 rounded-full bg-brand/10 blur-3xl" /><div className="relative"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="scout-eyebrow">AI teammate update</p><h2 id={headingId} className="scout-title mt-1">Scout briefing</h2></div><ScoutStatusBadge state={state} label={`${briefing.priority} priority`} /></div>
    ...
    <div className="mt-5 flex items-center justify-between gap-3"><ScoutPresence state={state} /><span className="text-xs text-muted">Updated <LocalTime value={briefing.generated_at} fallback="just now" /></span></div>
