...
export function ScoutRuntimePanel({ runtime, className }: { runtime?: ScoutRuntimeSnapshot | null; className?: string }) {
  ...
  return <section className={cn("scout-surface relative overflow-hidden rounded-3xl p-5 sm:p-6", className)} aria-labelledby="scout-runtime-heading"><div className="pointer-events-none absolute -right-16 -top-20 size-52 rounded-full bg-brand/10 blur-3xl" /><div className="relative"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="scout-eyebrow">Scout runtime</p><h2 id="scout-runtime-heading" className="scout-title mt-1">Live execution</h2></div><span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-xs text-muted"><Flag className="size-3.5 text-brand" />{runtime.executionStage}</span></div>
